...
Thirdly an additional method for implementing cel-shading, made more popular by modern visual node-shader tools such as Unity’s Shadergraph, is to use a “ramp texture”. It is a 1D gradient texture that maps an input scalar value to an output colour value. The “ramp” is sampled based on the lambert lighting information to define the transition between lit and shadowed areas of an object. This allows finer control over the placement of shadow bands, their size, their strength, and how harsh the shading is between each band, in an easier-to-visualise format.

Despite all these varying techniques, to create any cel-shaded shader, you always need the surface normal of an object and the view angle of the camera. This information can then be compared with a dot function to create the base lambert lighting model used in every method. In addition to the basic cel-shading method, various other techniques can be used to enhance the look of the scene. For example, line rendering can be used to add outlines to the objects in the scene, which can help to define the edges and give a more "hand-drawn" look. Additionally, texture maps can be used to add detailed patterns and textures to the objects, which can help to break up the flat, 2D appearance of the cels.
